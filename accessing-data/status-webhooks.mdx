...
put '/status-webhook' do
  # Retrieve the signature from the headers
  received_signature = request.env['HTTP_X_FUSE_SIGNATURE_256']

  # Compute a signature using the payload and your secret key
  secret_key = 'your_secret_key'
  expected_signature = OpenSSL::HMAC.hexdigest('sha256', secret_key, request.body.read)

  # Compare the signatures
  if received_signature != expected_signature
    return status 401, 'Mismatched signatures'
  end

  # Data is stored in the Fuse servers, let's consume it!
  if params['status'] == 'records_submitted'
    import_slug = params['import_id']
    import_records = JSON.parse(Net::HTTP.get(URI.parse("https://fuse.flatirons.com/api/v1/imports/#{import_slug}/records")))
    ...
    # Stored and processed! Let's update the Import status inside Fuse
    uri = URI.parse("https://fuse.flatirons.com/api/v1/webhooks/imports/#{import_slug}")
    http = Net::HTTP.new(uri.host, uri.port)
    request = Net::HTTP::Post.new(uri.request_uri)
    request['Authorization'] = 'YOUR_API_TOKEN'
    request.set_form_data('status' => 'records_processed')
