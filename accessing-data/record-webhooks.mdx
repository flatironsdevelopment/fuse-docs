...
In case of a batch failure, Fuse servers will retry X times until treat this as a failure. [add more info]

Depending on the amount of data you will be submitting, the batch size may vary. Your backend must support larger payload sizes in case you expect to submit large batches.

To do so, you may configure your backend to support a larger payload. Such as the examples below:
